// Copied from the pdf-file of the article:
//
// Jiangbo Huang,
...
//
// Source: https://arxiv.org/pdf/1410.1059
//
// AS:
// Did some refactoring of the original code.
...
int *MintermIndicesDecimal,
  *MintermIndicesDecimal_DontCare,
  **Minterm_Binary,
  ****Column,
  **PI_Index,
  **EPI_Index,
...
#define ERR_NREAD  -1 // File not readable
#define ERR_VRINV  -2 // Invalid number of variables
#define ERR_MTINV  -3 // Invalid number of minterms
#define ERR_DCINV  -4 // Invalid number of don't cares
#define ERR_MTORD  -5 // Minterm out of order
#define ERR_MTVAL  -6 // Minterm value error
#define ERR_DCORD  -7 // Don't care out of order
#define ERR_DCVAL  -8 // Don't care value error
#define ERR_NOMEM  -9 // Out of memory error

/********** A small garbage collector **********/
...
void gc_free()
{
  ...
  {
    if(GC_DEBUG) fprintf(stderr,"gc free at index %d\n",iptr);
    free(ptrs[iptr]);
  }
  if(GC_DEBUG) fprintf(stderr,"gc free ptrs\n");
  free(ptrs);
  iptr=0;
  ptrs=NULL;
  nptrs=NPTRS;
...
int ReadParamsFile(char *fname)
{
  ...
  if(inf==NULL)
  {
    printf("Cannot open file '%s' for readimng parameters.\n",fname);
    err=ERR_NREAD;
    goto errorexit;
  }

  if(EOF==fscanf(inf,"%d",&NumberOfVariable))
  {
    err=ERR_PMEOF;
    goto errorexit;
  }
  ...
  {
    printf("The number of variables should be greater than 0.\n");
    err=ERR_VRINV;
    goto errorexit;
  }

  if(EOF==fscanf(inf,"%d",&NumberOfAllMinterm))
  {
    err=ERR_PMEOF;
    goto errorexit;
  }
  ...
  if(EOF==fscanf(inf,"%d",&NumberOfDontCare))
  {
    err=ERR_PMEOF;
    goto errorexit;
  }
  if(NumberOfDontCare>=NumberOfAllMinterm || NumberOfDontCare<0)
  {
    printf("The number of Don't-Care minterms cannot be greater than the number of all minterms nor smaller than 0.\n");
    err=ERR_DCINV;
    goto errorexit;
  }
  ...
  MintermIndicesDecimal_DontCare=(int *)gc_malloc(NumberOfDontCare*sizeof(int));
  /* Record the decimal indices representing Don't-Care minterms */
  
  for(i=0;i<NumberOfAllMinterm;i++)
  {
    if(EOF==fscanf(inf,"%d",&MintermIndicesDecimal[i]))
    {
      err=ERR_PMEOF;
      goto errorexit;
    }
  ...
  if(NumberOfDontCare!=0)
  {
    ...
    {
      if(EOF==fscanf(inf,"%d",&MintermIndicesDecimal_DontCare[i]))
      {
        err=ERR_PMEOF;
        goto errorexit;
      }
  ...
  if(NumberOfDontCare!=0)
  {
    ...
    for(i=0;i<NumberOfDontCare;i++)
    {
      if(i==0)
        ...
        printf("Please enter the decimal index of the 2nd Don't-Care minterm (in ascending order):");
      else if(i==2)
        printf("Please enter the decimal index of the 3rd Don't-Care minterm (in ascending order):");
      else
...
int main(int argc, char *argv[])
{
  ...
  Column=(int ****)gc_malloc((NumberOfVariable+1)*sizeof(int***));

  for(i=0;i<NumberOfVariable+1;i++)
  {
    Column[i]=(int ***)gc_malloc((NumberOfVariable+1-i)*sizeof(int**));
  ...
  for(i=0;i<NumberOfVariable+1;i++)
  {
    ...
    {
      Column[i][j]=(int**)gc_malloc(Combination(NumberOfVariable,i,j)*sizeof(int*));
      /* Column[i][j] contains all the terms with j '1's in their binary indices in the (i+1)th column */
        ...
        for(k=0;k<Combination(NumberOfVariable,i,j);k++)
          if(Column[i][j][k]!=NULL)
          {
            for(l=0;l<Combination(NumberOfVariable,i,j+1);l++)
            {
              ...
              {
                LogicProbe=0-i;
                /*This LogicProbe is used to check whether this two terms has the same positions
                  of '-'(which is represented by '2')*/
                for(n=1;n<=i;n++)
                  for(p=1;p<=i;p++)
  ...
  PI_Index=(int **)gc_malloc(NumberOfAllMinterm*sizeof(int*));

  for(i=0;i<NumberOfAllMinterm;i++)
  {
    PI_Index[i]=(int *)gc_malloc(3*sizeof(int));
  }
  ...
  EPI_Index=(int **)gc_malloc(NumberOfAllMinterm*sizeof(int*));
      ...
      for(j=0;j<NumberOfPI;j++)
	    for (k = 0; k < ipow(2, PI_Index[j][0]); k++)
        {
          if(Column[PI_Index[j][0]]
                   [PI_Index[j][1]]
                   [PI_Index[j][2]]
                   [NumberOfVariable+3+PI_Index[j][0]+k]
             ==i)
          {
			...
			for (l = 0; l < ipow(2, PI_Index[j][0]); l++)
              NumberCounter[Column[PI_Index[j][0]]
                                  [PI_Index[j][1]]
                                  [PI_Index[j][2]]
                                  [NumberOfVariable+3+PI_Index[j][0]+l]]
              =0;
  ...
  ReducedPIChart_X=(int *)gc_malloc(NumberOfRemainingMT*sizeof(int));

  for(i=0;i<NumberOfRemainingMT;i++)
    ReducedPIChart_X[i]=-1;

  ReducedPIChart_Y=(int **)gc_malloc(NumberOfPI*sizeof(int*));

  for(i=0;i<NumberOfPI;i++)
    ReducedPIChart_Y[i]=NULL;

  ReducedPIChart=(int **)gc_malloc(NumberOfRemainingMT*sizeof(int*));
  ...
  /***********ReducedPIChart[i][j] represent the information of Reduced PI Chart
   ('1' means picked, '0' means unpicked)***********/
 
  if(NumberOfRemainingPI!=0)
  {
    for(i=0;i<NumberOfRemainingMT;i++)
      ReducedPIChart[i]=(int *)gc_malloc(NumberOfRemainingPI*sizeof(int));
	  ...
	  for (j = 0; j < ipow(2, ReducedPIChart_Y[i][0]); j++)
        for(k=0;k<NumberOfRemainingMT;k++)
          if(Column[ReducedPIChart_Y[i][0]]
                   ...
                   [ReducedPIChart_Y[i][2]]
                   [NumberOfVariable+3+ReducedPIChart_Y[i][0]+j]
             ==ReducedPIChart_X[k])
          {
    ...
    For=(int *)gc_malloc(NumberOfRemainingMT*sizeof(int));
    ...
    Potential_EPI=(int **)gc_malloc(NumberOfPossibleEPI*sizeof(int*));

    for(i=0;i<NumberOfPossibleEPI;i++)
    {
      Potential_EPI[i]=(int *)gc_malloc(NumberOfRemainingMT*sizeof(int));
    }

    Recursion_For_Loop(NumberOfRemainingMT-1);
    NoOfPIForEPI=(int *)gc_malloc(NumberOfPossibleEPI*sizeof(int));
    /* NoOfPIForEPI[i] will count how many PIs are in each combination that covers all minterms */
    for(i=0;i<NumberOfPossibleEPI;i++)
      NoOfPIForEPI[i]=0;
